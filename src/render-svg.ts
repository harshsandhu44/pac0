...
function pelletColor(type: CellNode['pelletType'], colors: Theme['colors']): string {
  switch (type) {
    case 'small':
      return colors.pelletSmall
    case 'medium':
      return colors.pelletMedium
    case 'large':
      return colors.pelletLarge
    case 'power':
      return colors.pelletPower
    default:
      return 'transparent'
  }
}
...
export function renderSVG(
  ...
  height="${height}"
  role="img"
  aria-label="${grid.username}'s GitHub contributions (last 365 days)">
  <title>${grid.username}'s GitHub contributions — last 365 days</title>
  <desc>A chomper traverses a pellet grid built from ${grid.username}'s GitHub contribution history over the last 365 days.</desc>
